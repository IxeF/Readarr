...
# correct UID/GID for NAS
ARG PUID=1000
ARG PGID=10   
# IMPORTANT: NAS admin group
...
###########################################################
# USER FIX — USE EXISTING GROUP 10 (wheel)
###########################################################

# Create user with correct UID and add to wheel (GID 10)
RUN adduser -D -u ${PUID} readarr && \
    addgroup readarr wheel && \
