...
###########################################################
# FIXED USER HANDLING — THIS IS THE IMPORTANT PART
###########################################################

# Create group and user matching host NAS permissions
RUN addgroup -g ${PGID} readarr && \
    adduser -D -u ${PUID} -G readarr readarr

# Fix ownership of all relevant directories
RUN chown -R ${PUID}:${PGID} /app /config /AudioBooks && \
